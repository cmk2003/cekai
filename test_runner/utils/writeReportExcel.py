...
def get_error_response_content(summary____details):
    """
    将json报告中的报错信息罗列出来
    :param summary____details: list summary["___details"]
    :return: content:[{
    ...
    temp_out_keys = []
    for testcases in summary____details:
...
def write_excel_log(summary):
    ...
    # 验证excelReport目录是否存在，如果不存在则创建该目录
    ...
    # 验证文件路径是否存在，如果不存在则生成该文件
    if not os.path.exists(excel_report_path):
        # 获取报告数据中的响应内容中的错误
        error_content, out_keys = get_error_response_content(summary["___details"])
